...
import { generateQuotationDocx } from "../../utils/generateQuotation";
        ...
        <tbody>
          {quotation.items.map((item, index) => (
            <tr key={index}>
              ...
              <td style={{ border: "1px solid #000", padding: "8px", textAlign: "right" }}>{item.total}</td>
...
{quotation.terms.map((term, index) => (
  <div key={index} style={{ display: "flex", marginBottom: "6px" }}>
    ...
    <div style={{ width: "150px" }}>{term.title}</div>
  ...
  {quotation.terms.map((term, index) => (
    ...
    >
      ...
      <input
        ...
        value={term.title}
        onChange={(e) => {
          const updated = [...quotation.terms];
          updated[index].title = e.target.value;
          setQuotation({ ...quotation, terms: updated });
        }}
        style={{
          width: "150px",
          border: "none",
      ...
      <button
        onClick={() => {
          const updated = quotation.terms.filter((_, i) => i !== index);
          setQuotation({ ...quotation, terms: updated });
        }}
  ...
  <button
    onClick={() =>
      setQuotation({
        ...quotation,
        terms: [...quotation.terms, { title: "", value: "" }],
      })
    }
